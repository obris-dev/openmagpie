...
- **Personal access tokens (headless login).** `sign_in_with_token(raw_token)` is the no-browser path (`magpie auth login --token`): it stores a long-lived `mgp_...` token via `Config.apply_personal_access_token` (which leaves `refresh_token` empty so the http layer sends the PAT directly and never tries to rotate it). A PAT is NOT revoked on `sign_out` (it is a durable, reusable credential); end its life explicitly with `magpie auth token revoke`. The secret is read from piped stdin or a hidden prompt, never an argv value.
- **`MAGPIE_TOKEN` is an AMBIENT credential.** `MagpieClient._ambient_token()` reads it on every request: when set it's the bearer (precedence over the stored login), and it's never persisted or refreshed (`_ensure_fresh_token` and the 401-retry both no-op for it). Whatever value it holds is sent as-is, so it should be a long-lived PAT, a short-lived session token would expire here un-refreshed (unlike a login-stored one, which rotates). So a box can `export MAGPIE_TOKEN=...` and skip `auth login` entirely, the standard env-var pattern (gh's `GH_TOKEN`). The commands are env-aware: `login` REFUSES while it's set (gh-style, the env token and a stored login are mutually exclusive), `logout` warns it keeps authenticating until unset, and `status` shows `Auth: MAGPIE_TOKEN (environment)`. `MAGPIE_TOKEN` is therefore NOT a `--token` input source (login would refuse first); `auth login --token` reads stdin or a hidden prompt.
