...
| `magpie feed source list` / `set` / `export` / `remove` / `get` / `template` | A feed's source set: `list` / `set` / `export` take `--feed <id>`; `remove` / `get` take the source's own id (the feed is resolved + confirmed for you); `template` emits a skeleton |
...
| `magpie watch action list` / `add` / `get` / `set` / `remove` | A watch's action chain: `list` / `add` take `--watch <id>`; `get` / `set` / `remove` take the action's own id (`get` shows one action's kind + config) |
