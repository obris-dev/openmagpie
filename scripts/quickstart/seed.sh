...
if docker compose exec -T core python -c "import os,urllib.request; urllib.request.urlopen(os.environ.get('OLLAMA_URL','')+'/api/tags', timeout=3)" >/dev/null 2>&1; then
    ...
    if manage poll_due_feeds && manage process_due_watches && manage process_due_runs && manage process_due_digests; then
        ...
        if [ -n "$aid" ]; then
            tail_msg="See the matched-vs-filtered breakdown: magpie watch action activity $aid (after magpie auth login)."
        else
            tail_msg="Re-check anytime: magpie watch action activity <action_id> (ids are in the seed summary above)."
        fi
        echo "Tick done. Posts that cleared the threshold printed above, tagged with the starter's prefix (e.g. [oss starter]); a backlog can also score zero on the first pass. $tail_msg"
    else
