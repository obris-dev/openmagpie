...
- A single action executing against one feed item is a **`WatchActionRun`** (the audit row), surfaced publicly (CLI + REST) as **`activity`** (the model name stays `WatchActionRun`; `run` reads as a verb in a command, the unit is still a run). There is no `Event` / hit model; a successful filter is just a `WatchActionRun` that advanced the chain.
